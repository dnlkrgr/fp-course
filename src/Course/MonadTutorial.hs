...
sequenceId = foldr
  (\a as -> bindId (\a' -> bindId (\as' -> pureId (a' : as')) as) a)
  (pureId [])
...
sequenceOptional = foldr
  (\a as ->
    bindOptional (\a' -> bindOptional (\as' -> pureOptional (a' : as')) as) a
  )
  (pureOptional [])
...
sequenceIntReader = foldr
  (\a as -> bindIntReader
    (\a' -> bindIntReader (\as' -> pureIntReader (a' : as')) as)
    a
  )
  (pureIntReader [])
...
sequenceReader = foldr
  (\a as -> bindReader (\a' -> bindReader (\as' -> pureReader (a' : as')) as) a)
  (pureReader [])
...
bindIntState f (IntState g) = IntState
  (\i ->
    let (a, j)     = g i
        IntState h = f a
    in  h j
...
sequenceIntState = foldr
  (\a as ->
    bindIntState (\a' -> bindIntState (\as' -> pureIntState (a' : as')) as) a
  )
  (pureIntState [])
...
sequenceState = foldr
  (\a as -> bindState (\a' -> bindState (\as' -> pureState (a' : as')) as) a)
  (pureState [])
...
sequenceOr = foldr
  (\a as -> bindOr (\a' -> bindOr (\as' -> pureOr (a' : as')) as) a)
  (pureOr [])
...
sequenceListFree = foldr
  (\a as ->
    bindListFree (\a' -> bindListFree (\as' -> pureListFree (a' : as')) as) a
  )
  (pureListFree [])
...
bindIntReaderFree
  :: (a -> IntReaderFree b) -> IntReaderFree a -> IntReaderFree b
...
sequenceIntReaderFree = foldr
  (\a as -> bindIntReaderFree
    (\a' -> bindIntReaderFree (\as' -> pureIntReaderFree (a' : as')) as)
    a
  )
  (pureIntReaderFree [])
...
sequenceReaderFree = foldr
  (\a as -> bindReaderFree
    (\a' -> bindReaderFree (\as' -> pureReaderFree (a' : as')) as)
    a
  )
  (pureReaderFree [])
...
sequenceFree = foldr
  (\a as -> bindFree (\a' -> bindFree (\as' -> pureFree (a' : as')) as) a)
  (pureFree [])
...
sequenceIO = foldr
  (\a as -> bindIO (\a' -> bindIO (\as' -> pureIO (a' : as')) as) a)
  (pureIO [])
